...
function App(){
 ...
 const [messages,setMessages]=useState<string[]>(["I'm ANDREAA CHAN'NEL AI Assist. Tell me the outcome you need and I'll route you to the right Ross PDF workflow."]);
 ...
 const ask=()=>{const q=chat.trim();if(!q)return;const l=q.toLowerCase();const route=l.includes('ocr')?'OCR':l.includes('convert')?'Conversions':l.includes('form')?'Forms':l.includes('billing')?'Billing':l.includes('security')?'Security':l.includes('template')?'Templates':'Workspace';setMessages(m=>[...m,'You: '+q,"AI: Routing to "+route+'. Confirm before any destructive action.']);setChat('')};
 return <div className={dark?'app dark':'app'}>
  <header className="topbar"><BrandLink/><div className="search"><input aria-label="Search tools, documents, commands" placeholder="Search tools, documents, commands…" value={query} onChange={e=>setQuery(e.target.value)}/></div><button onClick={()=>setDark(v=>!v)}>{dark?'Light':'Dark'}</button><button onClick={()=>setActive('account')}>Sign In</button><button className="gold" onClick={()=>setActive('billing')}>Start Free Trial</button></header>
  ...
  <section className="grid">{activeModule.actions.map(a=><div className="card" key={a}><h3>{a}</h3><p>Production action surface with explicit validation, authorization and audit boundaries.</p><button onClick={()=>setMessages(m=>[...m,'Action: '+a])}>Open</button></div>)}</section>
  {active==='templates'?<section className="card"><h2>150 Seeded Templates</h2><div className="chips">{templateCategories.flatMap(c=>templateNames.slice(0,12).map(n=><button key={c+'-'+n}>{c+' · '+n}</button>))}</div></section>:null}
